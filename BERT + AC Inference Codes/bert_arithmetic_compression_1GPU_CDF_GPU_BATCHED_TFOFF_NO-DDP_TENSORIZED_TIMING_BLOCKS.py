...
def load_CausalBERT(model_name = 'bert-base-cased', device = None):
	"""
	Loads the fine-tuned CausalBERT weights and biases from a saved checkpoints
	"""

	bert_model = CausalBERT(device = device, model_name = model_name)
	bert_state = torch.load('bert_ddp_trained_model_1GPU/pytorch_model_BERT_1GPU.bin', map_location = device)
...
def predict_next_token_probs(bert_model, batch_context_ids = None, batch_attention_masks = None, temperature = 1.0):
	"""
	...
	with torch.inference_mode():
		logits = bert_model(encoder_input_ids = batch_context_ids, encoder_attention_masks = batch_attention_masks) # Forward pass via CausalBERT
		step_logits = logits[:, -1, :] / temperature # Since CausalBERT predicts the next token autoregressively, we use the last context position
...
def evaluate_ac_bert(device = None, data_path = None, context_window = 64, temperature = 1.0):
	"""
	This function:
	...
	# Load CausalBERT model
	bert_model = load_CausalBERT(model_name = 'bert-base-cased', device = device)
	...
	for step in range(max_steps):
		...
		for batch in range(0, stream_ids_tensor.numel(), B_target):
            ...
            # Build BERT encoder contexts and attention masks from previously encoded tokens
			...
			prob_dist_batched = predict_next_token_probs(bert_model, batch_context_ids = context_ids, batch_attention_masks = attention_masks, temperature = temperature)
	...
	for step in range(max_steps):
		...
		for batch in range(0, stream_ids_tensor.numel(), B_target):
            ...
            # Build BERT encoder contexts and attention masks from decoded tokens
			context_ids = context_ids_batch
			attention_masks = torch.ones_like(context_ids, device = device, dtype = torch.long)
	...
	average_cross_entropy = total_log_likelihood / total_tokens
	perplexity = math.exp(average_cross_entropy)
	average_bpt = average_cross_entropy / math.log(2)
	average_kl_divergence = total_kl_divergence / total_tokens
...
if __name__ == '__main__':
	evaluate_ac_bert(device = 'cuda', data_path = 'wiki.train.txt', context_window = 64, temperature = 1.0)
