...
# at Southern Methodist University. It is intended for educational and research purposes only.
...
class CausalBERT(nn.Module):
	def __init__(self, device = None, model_name = 'bert-base-cased'):
		...
		self.bert = BertModel.from_pretrained(model_name, config = config, add_pooling_layer = False)
		self.lm_head = nn.Linear(config.hidden_size, config.vocab_size, bias = False)
		self.to(self.device)

	def forward(self, encoder_input_ids = None, encoder_attention_masks = None, labels = None):
		outputs = self.bert(input_ids = encoder_input_ids, attention_mask = encoder_attention_masks)
...
def main_worker(rank, world_size, local_rank, epochs = 10, batch_size = 128, lr = 1e-5, max_norm = 1.0):
	...
	model = train_bert(dataloader, train_sampler, device, rank, local_rank, epochs, lr, max_norm)
	end_time = time.time()
	...
	if rank == 0:
		print(f'Total wall clock time for fine-tuning BERT: {wall_clock_time:.4f} seconds', flush = True)
		save_model(model, 'bert_ddp_trained_model_12GPUs')
	clean_process()
...
def save_model(model, save_path):
	...
	torch.save(model_to_save.state_dict(), f'{save_path}/pytorch_model_BERT_12GPUs.bin')
	print(f'Fine-tuned BERT checkpoints saved to {save_path}')
