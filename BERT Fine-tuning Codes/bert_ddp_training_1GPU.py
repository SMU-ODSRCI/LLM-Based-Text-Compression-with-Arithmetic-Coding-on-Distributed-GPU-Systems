...
# BERT fine-tuning for next-token prediction
...
def main_worker(rank, world_size, local_rank, epochs = 10, batch_size = 128, lr = 1e-5, max_norm = 1.0):
	...
	if rank == 0:
		print(f'Total wall clock time for fine-tuning BERT: {total_time:.4f} seconds', flush = True)
		save_model(model, 'bert_ddp_trained_model_1GPU')
	clean_process()
...
def save_model(model, save_path):
	...
	torch.save(model_to_save.state_dict(), f'{save_path}/pytorch_model_BERT_1GPU.bin')
