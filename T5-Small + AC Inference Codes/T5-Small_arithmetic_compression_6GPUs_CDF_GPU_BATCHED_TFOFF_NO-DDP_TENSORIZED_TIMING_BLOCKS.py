...
# Here, CausalT5 refers to the T5-Small model
...
def predict_next_token_probs(t5small_model, batch_encoder_context = None, batch_encoder_attention_mask = None, batch_decoder_context = None, batch_decoder_attention_mask = None, temperature = 1.0):
	...
	with torch.inference_mode():
		...
		step_logits = outputs.logits[:, -1, :] / temperature # Since CausalT5 predicts the next token autoregressively, we use the last context position
...
def evaluate_ac_t5(rank, world_size, local_rank, data_path = 'wiki.train.txt', context_window = 64, temperature = 1.0):
	...
	# Tokenize the text and load tokenized data via manual sharding
	...
	# Build a tensor of local stream IDs outside the hot loop so that indexing uses local_strean_id directly
	...
	for step in range(max_steps):
		...
		for batch in range(0, local_stream_ids_tensor.numel(), stride):
			...
			context_batch = encoder_context_all[active_local_stream]
			...
			decoder_context[:, 1:] = encoder_context[:, :-1]
	...
	for step in range(max_steps):
		...
		for batch in range(0, local_stream_ids_tensor.numel(), stride):
			...
			context_batch = decoder_context_all[active_local_stream]
	...
	total_encoding_time = encoder_end - encoder_start
	total_decoding_time = decoder_end - decoder_start
...
def main_worker(args):
	...
	evaluate_ac_t5(rank, world_size, local_rank, **args)
	clean_process()
