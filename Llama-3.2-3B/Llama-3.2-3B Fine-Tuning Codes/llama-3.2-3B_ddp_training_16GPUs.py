...
def distributed_config(rank, world_size, local_rank):
	...
	dist.init_process_group(backend = 'nccl', init_method = f'tcp://{host}:29509', rank = rank, world_size = world_size)
...
def save_model(llama_model, save_path):
	...
	torch.save(model_to_save.state_dict(), f'{save_path}/pytorch_model_Llama-3.2-3B_16GPUs.bin')
...
def main_worker(rank, world_size, local_rank, epochs = 10, batch_size = 128, lr = 1e-5, max_norm = 1.0, fused_AdamW = True, torch_compile = True):
	...
	if rank == 0:
		...
		save_model(llama_model, 'Llama-3.2-3B_finetuned_16GPUs')
	clean_process()
