...
def distributed_config(rank, world_size, local_rank):
	...
	dist.init_process_group(backend = 'nccl', init_method = f'tcp://{host}:29508', rank = rank, world_size = world_size)
...
def main_worker(rank, world_size, local_rank, epochs = 10, batch_size = 128, lr = 1e-5, max_norm = 1.0):
	...
	if rank == 0:
		...
		save_model(model, 'bert_ddp_trained_model_16GPUs')
	clean_process()
...
def save_model(model, save_path):
	...
	torch.save(model_to_save.state_dict(), f'{save_path}/pytorch_model_BERT_16GPUs.bin')
