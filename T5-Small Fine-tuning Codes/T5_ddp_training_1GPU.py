...
def main_worker(rank, world_size, local_rank, epochs = 10, batch_size = 128, lr = 1e-5, max_norm = 1.0):
	...
	if rank == 0:
		print(f'Total wall clock time for fine-tuning T5-Small: {total_time:.4f} seconds', flush = True)
		save_model(t5model, 'T5-Small_ddp_trained_model_1GPU')
	clean_process()
...
def save_model(t5model, save_path):
	...
	torch.save(model_to_save.state_dict(), f'{save_path}/pytorch_model_T5-Small_1GPU.bin')
