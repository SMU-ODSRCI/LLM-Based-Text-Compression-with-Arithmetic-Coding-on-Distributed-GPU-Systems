...
def main_worker(rank, world_size, local_rank, epochs = 10, batch_size = 128, lr = 1e-5, max_norm = 1.0):
	...
	if rank == 0:
		print(f'Total wall clock time for fine-tuning RoBERTa: {wall_clock_time:.4f} seconds', flush = True)
		save_model(model, 'roberta_ddp_trained_model_12GPUs')
	clean_process()
...
def save_model(model, save_path):
	...
	torch.save(model_to_save.state_dict(), f'{save_path}/pytorch_model_RoBERTa_12GPUs.bin')
