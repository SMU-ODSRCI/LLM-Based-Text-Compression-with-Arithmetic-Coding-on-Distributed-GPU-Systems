...
def save_model(llama_model, save_path):
	...
	torch.save(model_to_save.state_dict(), f'{save_path}/pytorch_model_Llama-3.2-3B_1GPU.bin')
...
def main_worker(rank, world_size, local_rank, epochs = 10, batch_size = 128, lr = 1e-5, max_norm = 1.0, fused_AdamW = True, torch_compile = True):
	...
	if rank == 0:
		print(f'Total wall clock time for fine-tuning Llama-3.2-3B: {total_time:.4f} seconds', flush = True)
		save_model(llama_model, 'Llama-3.2-3B_finetuned_1GPU')
	clean_process()
