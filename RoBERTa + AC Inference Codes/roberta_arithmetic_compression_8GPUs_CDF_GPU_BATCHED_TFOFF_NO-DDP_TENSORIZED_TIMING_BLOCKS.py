...
def distributed_config(rank, world_size, local_rank):
	...
	dist.init_process_group(backend = 'nccl', init_method = f'tcp://{host}:29503', rank = rank, world_size = world_size)
...
def load_CausalRoBERTa(model_name = 'roberta-base', device = None):
	...
	roberta_state = torch.load('roberta_ddp_trained_model_1GPU/pytorch_model_RoBERTa_1GPU.bin', map_location = device)
...
# Compute next-token probabilities using bert_model
def predict_next_token_probs(roberta_model, batch_context_ids = None, batch_attention_masks = None, temperature = 1.0):
	"""
	...
	with torch.inference_mode():
		logits = roberta_model(encoder_input_ids = batch_context_ids, encoder_attention_masks = batch_attention_masks) # Forward pass via CausalRoBERTa
		step_logits = logits[:, -1, :] / temperature # Since CausalRoBERTa predicts the next token autoregressively, we use the last context position
...
def evaluate_ac_roberta(rank, world_size, local_rank, data_path = 'wiki.train.txt', context_window = 64, temperature = 1.0):
	"""
	This function:
	...
	roberta_model = load_CausalRoBERTa(model_name = 'roberta-base', device = device)
	...
	stride = 128 # This allows the second inner batch loop to advance forward exactly by the no. of batched streams fed to each GPU
	...
	for step in range(max_steps):
		...
		for batch in range(0, local_stream_ids_tensor.numel(), stride):
            ...
            # Build RoBERTa encoder contexts and attention masks from previously encoded tokens
			...
			prob_dist_batched = predict_next_token_probs(roberta_model, batch_context_ids = context_ids, batch_attention_masks = attention_masks, temperature = temperature)
	...
	for step in range(max_steps):
		...
		for batch in range(0, local_stream_ids_tensor.numel(), stride):
            ...
            # Build RoBERTa encoder contexts and attention masks from decoded tokens
			context_ids = context_ids_batch
			attention_masks = torch.ones_like(context_ids, device = device, dtype = torch.long)
...
def main_worker(args):
	...
	evaluate_ac_roberta(rank, world_size, local_rank, **args)
	clean_process()
